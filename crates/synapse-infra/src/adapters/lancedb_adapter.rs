...
use synapse_core::{CoreError, MemoryNode, MemoryPort, SearchResult};
...
impl MemoryPort for LanceDbAdapter {
    async fn store(&self, _node: MemoryNode) -> Result<String, CoreError> {
        todo!("Implement LanceDB store - see FEAT_lancedb-adapter.md")
    }
    
    async fn search(&self, _embedding: &[f32], _top_k: usize) -> Result<Vec<SearchResult>, CoreError> {
    ...
    async fn get_by_id(&self, _id: &str) -> Result<Option<MemoryNode>, CoreError> {
        todo!("Implement LanceDB get_by_id")
    }
    
    async fn get_by_layer(&self, _layer: u8) -> Result<Vec<MemoryNode>, CoreError> {
        todo!("Implement LanceDB get_by_layer")
    }
    
    async fn update(&self, _node: MemoryNode) -> Result<(), CoreError> {
